...
window.addEventListener('load', () => {
    fetchBooks()
        .then(data => {
            // Data is already parsed JSON array from Lambda
            const formattedData = data.map((book) => ({
                id: book.id,
                picture: book.cover,  // Lambda uses 'cover' field
                title: book.title,
                author: book.author,
...
// Get API URL from config (generated by Terraform)
// Fallback to hardcoded URL for local development
const apiUrl = (window.APP_CONFIG && window.APP_CONFIG.API_URL) || 'https://hdhido0jt9.execute-api.us-east-1.amazonaws.com/dev/books';
        ...
        for(var i=tab_start; i<tab_end; i++){
            ...
            if(book){
                ...
                <td>
                    ...
                    <button onclick="editInfo('${book.id}', '${book.picture}', '${book.title}', '${book.author}', '${book.publisher}', '${book.year}')"><i class="fa-regular fa-pen-to-square"></i></button>

                    <button onclick="deleteInfo('${book.id}')"><i class="fa-regular fa-trash-can"></i></button>
                </td>
            </tr>`;
...
function editInfo(id, pic, Title, Author, Publisher, Year){
    ...
    // Find the book by ID in originalData
    ...
    editId = originalIndex;  // Store the index for later use in form submission
...
function deleteInfo(bookId){
    if(confirm("Are you sure you want to delete?")){
        // Find the book by ID
        const bookIndex = originalData.findIndex(item => item.id === bookId);
        if (bookIndex === -1) {
            console.error('Book not found with ID:', bookId);
            return;
        }
        
        const deletedItem = originalData.splice(bookIndex, 1);
        ...
        // API call - send id in lowercase format matching Lambda
        const deleteData = {
            id: String(deletedItem[0].id)
        }
...
form.addEventListener('submit', async (e) => {
    ...
    const information = {
        id: isEdit ? originalData[editId].id : Date.now(),  // Use existing id when editing
    ...
    try {
        if (!isEdit) {
            ...
            // API call - send data directly with lowercase fields matching Lambda
            const book = {
                ...
                cover: information.picture  // Lambda uses 'cover' field
            }
            await addBook(book)  // Wait for API call to complete
            ...
            // API call - send data directly with lowercase fields matching Lambda
            const updatedBook = {
                id: String(information.id),  // Use existing book id when editing
                title: information.title,
                author: information.author,
                publisher: information.publisher,
                year: information.year,
            ...
            await updateBook(updatedBook)  // Wait for API call to complete
        }
    } catch (error) {
        console.error('Failed to save book to database:', error);
        alert('Failed to save book to database. Please check the console for details.');
        // Optionally remove from local storage if API call failed
        if (!isEdit) {
            originalData.shift();  // Remove the item we just added locally
        }
        return;  // Don't proceed with form reset if API call failed
    }
    getData = [...originalData];
    localStorage.setItem('bookProfile', JSON.stringify(originalData));
...
async function deleteBook(deleteData) {
    try {
